	...
	public:	   face(void) { nv = 0; v = NULL; }
		  ...
		  return res; }
		   ...
		   void Imprimir(void) { printf("nvf: %d\n",nv);
	...
	public:	   solido(void) { nf = 0; f = NULL; }
		   ...
		   InfoGeo Intersecao(XYZ pos, XYZ dir) {
		      ...
		      if (!f)
		      { res1.ok = 0; }
		      else { res1 = f[0].Intersecao(pos,dir);
		      for ( int i=1; i<nf; i++ )
		   ...
		   void Imprimir(void) { if (f) for (int i=0; i<nf; i++ ) f[i].Imprimir(); }
	...
	public:	   matriz(void) {}
		   ...
		   void Registrar(XYZ r, XYZ s) {
			 ...
			 if (i>=0 && i<ny && j>=0 && j<nx) { ctg[i*nx+j]++; }
	...
	public:	   fonte(void) { nc = 0; theta = 0; spe = NULL; en = NULL; }
		   ...
		   void Ler_Dados(const char *nome_spe) {
		      ...
		      if (!fp) { printf("Erro ao abrir arquivo de espectro %s!!!\n",nome_spe);
			         exit(0); }
		   ...
		   foton Emitir(void) {
		      ...
		      if (!nc) { printf("Espectro não definido!!!\n");
			         exit(0); }
	...
	typedef struct { double pabs, pee, pei, s; } InfoDB;
	...
	int main(int argc, char *argv[]) {
	   ...
	   for ( i=0; i<nev; i++ ) {
	      ...
	      if (pt.ok) { // printf(" (entrou)\tq: "); pt.q.Imprimir();
		 	   // colocar o foton sobre o modelo
		 	   hv.Pos(pt.q);
		 	   // loop de interacoes
		 	   
	      while (hv.Hv()>0.5) { // determina ponto de saida
		    		    pt = sol.Intersecao(hv.Pos(),hv.Dir());
				    // printf("\tok: %d",pt.ok);
	
   	      if (pt.ok) {   // printf(" \t\tq: "); pt.q.Imprimir();
		             // calcula passo e probabilidades
		       	     prob = db.Informacao(hv.Hv());
		       	     x = (pt.q-hv.Pos()).Norma();
		       
	      if (prob.s>x) {  // printf("\t\tsem interacao\n");
		               // colocar o foton sobre em pt.q
		               hv.Pos(pt.q);
			       // hv.Imprimir();
			    }
		       
	      else {	// atualiza posicao do foton
		        hv.Pos(hv.Pos()+hv.Dir()*prob.s);
			// hv.Imprimir();
		        // sorteia mecanismo de interacao
		        eps = rand()/(double)RAND_MAX;
		        if (eps<prob.pabs)
		        { db.AbsFotoeletrica(&hv); } // printf("\t\tabsorcao fotoeletrica\n");
		        else if (eps<prob.pabs+prob.pee)
		        { db.EspalhamentoElastico(&hv); } //printf("\t\tespalhamento elastico\n");
		        else { db.EspalhamentoInelastico(&hv); } // printf("\t\tespalhamento inelastico\n");
		        }
		     }
		    
		   else { break; } // printf(" (saiu)\n");
		}
	     }
	      ...
	      if (hv.Hv()>0.5) { det.Registrar(hv.Pos(),hv.Dir()); } // testar se chegou no detector
